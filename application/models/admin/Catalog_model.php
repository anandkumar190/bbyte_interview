...
{ 
	...
	public function save_category($post)
	{
		...
		if(!$order_number)
		{
			$get_last_order_number = $this->db->select('order_number')->order_by('order_number','desc')->get_where('category',array('parent_id'=>$post['parent_cat_id']))->row_array();
			$order_number = isset($get_last_order_number['order_number']) ? $get_last_order_number['order_number'] + 1 : 1;
		}
		...
		$data = array(
			...
			'status' => $post['status'],
			'is_cranes_choice' => isset($post['is_cranes_choice']) ? 1 : 0,
