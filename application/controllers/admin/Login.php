...
$sms_url = "http://bulksms.thedigitalindia.net/index.php/smsapi/httpapi/?uname=cranesmart&password=123456&sender=CRANES&receiver=".$status['mobile']."&route=TA&msgtype=1&sms=Your%20OTP%20".$otp."%20For%20Login%20Account";
	...
	function getRecords($url)
	{
        ...
        //print_r($objResponse);
        $enter_otp_data =$this->db->query("UPDATE `tbl_users` SET `otp` = '$otp',`updated`=now() WHERE (username = '$username' or mobile = '$username'  or email = '$username') ");
				...
				if($status['role_id'] == 1){
					...
					$this->session->set_flashdata('Otp',1);
					$this->session->set_userdata('username',$username);
					$this->session->set_userdata('password',$password);
					
					$this->Az->redirect('admin/login');
...
public function VerifyOtploginAuth()
	{
		...
		{
			// check username or password
			$status = $this->Login_model->adminLoginAuthentication_OTP($post);
			if($status)
			{
				...
				if($status['role_id'] == 1){
                	...
                	$this->session->set_flashdata($Otp_error,1);
					$this->Az->redirect('admin/login', 'system_message_error', lang('FRONT_LOGIN_ACCESS_DENIED'));
				}
			}
