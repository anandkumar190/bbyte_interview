...
class Kyc extends CI_Controller {    
	...
	public function updateKYCAuth($requestID = 0, $status = 0)
	{
		...
		if(!$chk_request_id)
		{
			$this->Az->redirect('admin/kyc/kycList', 'system_message_error',lang('DB_ERROR'));
		}
		
		$this->Kyc_model->updateRequestAuth($requestID,$status);
		if($status == 1){
			$this->Az->redirect('admin/kyc/kycList', 'system_message_error',lang('REQUEST_APPROVE_SUCCESS'));
		}
		else
		{
			$this->Az->redirect('admin/kyc/kycList', 'system_message_error',lang('REQUEST_REJECT_SUCCESS'));
		}
	}
