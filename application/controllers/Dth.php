...
class Dth extends CI_Controller{
	...
	public function index(){
		...
		$operator = $this->db->get_where('operator',array('type'=>'DTH'))->result_array();
    ...
    {   
        ...
        {   
            ...
            if($chk_wallet_balance['wallet_balance'] < $post['amount']){
				...
				$this->Az->redirect('dth', 'system_message_error',lang('WALLET_ERROR'));   

            }  
            
            //$this->Az->redirect('dth', 'system_message_error',lang('RECHARGE_DOWN_ERROR'));   
            ...
            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
			{
				...
				if($api_response['status'] == 1){
					...
					$this->Az->redirect('dth', 'system_message_error',lang('RECHARGE_PENDING'));
				}
				else
				{
					// update recharge status
					...
					// add 100% CM Points
					$this->User->recharge_cm_points_add($loggedUser['id'],$post['amount'],'DTH');
					
					$this->Az->redirect('dth', 'system_message_error',lang('RECHARGE_SUCCESS'));
				}
			}
			else
			{
				...
				$this->Az->redirect('dth', 'system_message_error',lang('RECHARGE_FAILED'));
			}
