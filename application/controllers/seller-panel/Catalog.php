...
class Catalog extends CI_Controller {    
	...
	public function getProductList()
	{
		...
		if($get_filter_data){
			foreach($get_filter_data as $list){
				...
				$nestedData[] = '<a href="#" class="btn btn-primary" onclick="openStockModal('.$list['id'].')">Manage</a>';
				if($list['status']) {
				...
				$nestedData[] = '<a title="edit" class="btn btn-primary btn-sm" href="'.base_url('seller-panel/catalog/editProduct').'/'.$list['id'].'"><i class="fa fa-edit" aria-hidden="true"></i></a>';
				$nestedData[] = '<a title="delete" class="btn btn-danger btn-sm" href="'.base_url('seller-panel/catalog/deleteProduct').'/'.$list['id'].'" onclick="return confirm(\'Are you sure you want to delete?\')"><i class="fa fa-trash" aria-hidden="true"></i></a>';
				$data[] = $nestedData;
	...
	public function saveProduct()
	{
		...
		else
		{
			...
			$this->Az->redirect('seller-panel/catalog/productList', 'system_message_error',lang('PRODUCT_SAVE_SUCCESS'));
		}
	...
	public function deleteProduct($product_id = 0, $uploadError = '')
	{
		...
		$chk_cat = $this->db->get_where('products',array('id'=>$product_id,'account_id'=>$account_id))->num_rows();
		if(!$chk_cat)
		{
			$this->Az->redirect('seller-panel/catalog/productList', 'system_message_error',lang('PRODUCT_VALID_ERROR'));
		}
		...
		$this->Az->redirect('seller-panel/catalog/productList', 'system_message_info', lang('PRODUCT_DELETE_SUCCESS'));
