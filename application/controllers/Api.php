...
class Api extends CI_Controller{
	...
	public function userRegAuth()
	{
		...
		else
		{
			...
			else
			{
				...
				$referral_id = isset($post['referral_id']) ? trim($post['referral_id']) : '';
    ...
    public function rechargeAuth(){
		...
		else
		{
		    $response = array(
				'status' => 0,
				'message' => 'Recharge is down due to technical issue.'
			);
			$userID = $post['userID'];
			$type = $post['type'];
			/* Type = 1 for Mobile */
							...
							else
							{
					            ...
					            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
								{
									...
									else
									{
										...
										// add 100% CM Points
										$this->User->recharge_cm_points_add($userID,$post['amount'],'Mobile',$recharge_unique_id);
						...
						else
						{
				            ...
				            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
							{
								...
								else
								{
									...
									// add 100% CM Points
									$this->User->recharge_cm_points_add($userID,$post['amount'],'Mobile');
