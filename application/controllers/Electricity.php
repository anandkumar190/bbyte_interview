...
class Electricity extends CI_Controller{
	...
	public function index(){
		...
		$operator = $this->db->get_where('operator',array('type'=>'Electricity'))->result_array();
           ...
           // $this->Az->redirect('electricity', 'system_message_error',lang('RECHARGE_DOWN_ERROR'));   
            ...
            if(isset($api_response['status']) && ($api_response['status'] == 1 || $api_response['status'] == 2))
			{
				...
				else
				{
					...
					// add 100% CM Points
					$this->User->recharge_cm_points_add($loggedUser['id'],$post['amount'],'Electricity');
